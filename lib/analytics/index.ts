...
/** goodday 이벤트 (analytics-spec.md 공통 스키마) */
export type GooddayEvent =
  | "landing_view"
  | "input_submit"
  | "result_view"
  | "share_click"
...
export function track(event: GooddayEvent, props: Record<string, unknown> = {}): void {
  if (typeof window === "undefined") return;
  try {
    window.posthog?.capture(event, { app: "goodday", ...props });
  } catch {
    /* 계측 실패는 조용히 무시 */
  }
}
